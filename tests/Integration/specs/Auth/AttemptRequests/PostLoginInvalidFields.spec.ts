...
const { route, payload } = buildAuthMock('post_login');

describeInvalidFieldSuite(test, {
  descricao: 'Attempt POST /auth/login - Campos Inválidos',
  route,
  ...
  scenarios: [
    ...
    ['senha',    INVALID.EMPTY,          HTTP.UNPROCESSABLE, 'Senha é obrigatória'],
    ['senha',    INVALID.NULL,           HTTP.UNPROCESSABLE, 'Senha é obrigatória'],
  ],
});
