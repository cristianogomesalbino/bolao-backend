...
describe('PontuacaoService', () => {
  ...
  describe('calcular', () => {
    it('deve retornar ACERTO_EM_CHEIO (3 pontos) quando palpite é igual ao placar do jogo', () => {
    ...
    it('deve retornar ACERTO_DE_RESULTADO (1 ponto) quando acerta vitória casa sem placar exato', () => {
      const resultado = service.calcular(
      ...
      expect(resultado.pontosBase).toBe(1);
    });

    it('deve retornar ACERTO_DE_RESULTADO (1 ponto) quando acerta empate com placar diferente', () => {
      const resultado = service.calcular(
    ...
    it('deve retornar ERRO_TOTAL (0 pontos) quando acerta gols de um time mas erra resultado', () => {
      const resultado = service.calcular(
        { golsCasa: 2, golsFora: 1 },
