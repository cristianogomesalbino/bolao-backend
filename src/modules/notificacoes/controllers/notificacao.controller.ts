import { Controller, Get, Patch, Param, Query, HttpCode } from '@nestjs/common';
import {
  ApiTags,
  ApiOperation,
  ApiResponse,
...
export class NotificacaoController {
  ...
  async listar(
    ...
    const resultado = await this.notificacaoService.listar(usuario.id, filtros);

    return {
      notificacoes: resultado.notificacoes.map((n) =>
        NotificacaoPresenter.toHttp(n),
      ),
