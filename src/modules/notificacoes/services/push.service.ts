...
export class PushService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    if (!publicKey || !privateKey) {
      this.logger.warn(
        'VAPID keys não configuradas — Web Push desabilitado',
      );
      return;
    }
  ...
  async inscrever(usuarioId: string, endpoint: string, p256dh: string, auth: string): Promise<void> {
    ...
    const totalInscricoes = await this.inscricaoRepo.contarPorUsuario(usuarioId);
  ...
  async enviarParaUsuario(usuarioId: string, payload: PushPayload): Promise<void> {
    ...
    const pushPayload = JSON.stringify({
      ...
      body: payload.mensagem.substring(0, NOTIFICACOES.LIMITES.PUSH_MENSAGEM_MAX),
      type: payload.tipo,
      url: payload.url,
    });
  ...
  async enviarParaUsuarios(usuarioIds: string[], payload: PushPayload): Promise<void> {
    if (!this.pushHabilitado) return;
  ...
  private async enviarPush(inscricao: InscricaoPush, payload: string): Promise<void> {
    const subscription = {
