...
export class RankingController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Ranking da fase retornado com sucesso.',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    type: Number,
    description: 'Ranking apenas da rodada específica',
  })
  @ApiQuery({
    name: 'ateRodada',
    required: false,
    type: Number,
    description: 'Ranking acumulado até a rodada (inclusive)',
  })
  @UseGuards(GroupRoleGuard)
  @GroupRoles(GRUPO_ROLE.ADMIN, GRUPO_ROLE.MEMBER)
  ...
  async obterRankingFase(
    ...
    const ranking = await this.rankingService.obterRankingFase(
      grupoId,
      faseId,
      rodadaNum,
      ateRodadaNum,
  ...
  @ApiOperation({
    summary: 'Obter detalhamento de pontuação de um jogo no grupo',
  })
  @ApiResponse({
    status: 200,
    description: 'Detalhamento de pontuação retornado com sucesso.',
  })
  ...
  async obterDetalhamentoJogo(
    ...
    const detalhamento = await this.rankingService.obterDetalhamentoJogo(
      grupoId,
      jogoId,
    );
